...
class HistoricalDataManager {
    ...
    async createDailySnapshotIfNeeded() {
        ...
        if (!todaySnapshot) {
            // Read current data to create snapshot
            try {
                const currentData = await this.readCurrentData();
                await this.createSnapshot(currentData, 'daily_auto');
            } catch (error) {
    ...
    async readCurrentData() {
        try {
            ...
            const shoppingFile = path.join(this.dataDir, 'shopping.json');
            ...
            try {
                ...
                console.warn('Could not read tasks file:', error.message);
            }
            
            try {
                const shoppingData = await fs.readFile(shoppingFile, 'utf8');
                shopping = JSON.parse(shoppingData);
            } catch (error) {
                console.warn('Could not read shopping file:', error.message);
            }
            ...
            return { tasks: [], shopping: [], version: this.currentVersion };
        }
    }
